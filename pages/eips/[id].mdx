  ...
  <Container maxWidth="lg">
    ...
    <Box sx={{ display: 'flex' }}>
      <Box sx={{ width: '840px' }} mr={3}>
        ...
        <Box sx={{display: 'flex'}}>
          {meta['extended resources'].map(item => (<Box sx={{width: '404px', marginBottom: '52px'}} mr={4} key={item.title}>
            ...
            <div> <Link href={item.link} sx={{textDecoration: 'none'}}>Learn more →</Link></div>
          </Box>))}
        </Box></>)}
      </Box>
        ...
        {meta.projects?.length > 0 && <><Box sx={{flex: 1}}>
          <Box sx={{height: 'auto', padding: '24px 24px 0', border: '1px solid #eaebf0', borderRadius: '10px'}}>
            ...
            {meta.projects.map(item => (<>
              <Box sx={{height: '100px', background: '#f3f3f3'}}  key={item.title}>
                <img style={{display: 'block', width: '100%', height: '100%', border: 'none'}} src={item.imgSrc} alt={item.alt} />
              </Box>
...
export async function getServerSideProps(context) {
  ...
  let originalEIP;
  try {
    originalEIP = await readFile(`./public/original-eips/${id}.md`, 'utf8');
  } catch (e) {
    console.log(e);
  }
  let meta, con;
  ...
  try {
    let comEIP = await readFile(`./public/content/en/${id}.md`, 'utf8');
